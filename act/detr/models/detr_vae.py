...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names,velocity_control = False, context_length=1):
        ...
        if backbones is not None:
            ...
            self.input_proj_robot_state = nn.Linear(8*self.context_length, hidden_dim)
            if velocity_control:
                self.input_proj_robot_state_vel = nn.Linear(8*self.context_length, hidden_dim)  # project concatenated [qpos, qvel]
        else:
        ...
        self.encoder_joint_proj = nn.Linear(8*self.context_length, hidden_dim)  # project qpos to embedding
        if velocity_control:
            self.encoder_joint_vel_proj = nn.Linear(8*self.context_length, hidden_dim) # project action to embedding
        ...
        additional_pos_size = 3 if velocity_control else 2
    ...
    def forward(self, qpos, image, env_state, qvel=None, actions=None, is_pad=None):
        ...
        if is_training:            # project action sequence to embedding dim, and concat with a CLS token
            ...
            if qvel is not None:
                print("Dimensions in VAE encoder:", cls_embed.shape, qpos_embed.shape, qvel_embed.shape, action_embed.shape)
        ...
        if self.backbones is not None:
            ...
            hs = self.transformer(src, None, self.query_embed.weight, pos, latent_input, proprio_input_pos, self.additional_pos_embed.weight, velocity_input = proprio_input_vel)[0]
        else:
