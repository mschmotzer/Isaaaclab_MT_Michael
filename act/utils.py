...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cache_mode == 'full' and episode_id in self.cache:
            ...
            max_start = max(0, episode_len - self.chunk_size )
            ...
            image_dict = {cam: data['images'][cam][start_ts] for cam in self.camera_names}
            action = data['actions'][start_ts:start_ts + self.chunk_size]
            ...
            with h5py.File(dataset_path, 'r') as root:
                ...
                max_start = max(0, episode_len - self.chunk_size - self.context_length + 1)
                start_ts = np.random.randint(0, max_start + 1) if max_start > 0 else 0
...
def load_data(dataset_dir, num_episodes, camera_names, batch_size_train, batch_size_val,chunk_size = 100,context_length=1, velocity_control=False):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size_train, shuffle=True, pin_memory=True, num_workers=20, prefetch_factor=4, persistent_workers=True)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size_val, shuffle=True, pin_memory=True, num_workers=20, prefetch_factor=4, persistent_workers=True)
